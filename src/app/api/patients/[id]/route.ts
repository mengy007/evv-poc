...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [rows] = await pool.query('SELECT id, hash, name FROM Patient WHERE id = ?', [id]);
    const row = (rows as any[])[0];
    return row ? ok(row) : notFound();
  } catch (e) {
    return serverError(e, 'Failed to fetch patient');
  }
}
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const parsed = UpdatePatient.safeParse(await req.json());
    ...
    return serverError(e, 'Failed to update patient');
  }
}
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return serverError(e, 'Failed to delete patient');
  }
}
